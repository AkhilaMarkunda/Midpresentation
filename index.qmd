...
{r,echo = TRUE} \# Reading and Preparing Data retail_sales \<- read.csv("C:/Users/Akhila Markunda/Downloads/RSXFS.csv") retail_sales \<- na.omit(retail_sales) consumer_conf \<- read.csv("C:/Users/Akhila Markunda/Downloads/CSCICP03USM665S.csv") merged_data \<- merge(retail_sales, consumer_conf, by = "DATE") merged_data$DATE <- as.Date(merged_data$DATE) merged_data_f \<- merged_data\[merged_data\$DATE \<= as.Date("2024-01-01"), \]
...
{r,echo = TRUE} library(ggplot2) ggplot(merged_data_f, aes(x = DATE, y = CSCICP03USM665S)) + geom_line() + labs(title = "Consumer confidence", x = "Year", y = "Normalized OECD")

## Retail sales

{r,echo = TRUE} ggplot(merged_data_f, aes(x = DATE, y = RSXFS)) + geom_line() + labs(title = "Retail sales", x = "Year", y = "Dollars")

## Correlating the Data

{r,echo = TRUE} \# Correlation correlation \<- cor(merged_data_f$RSXFS, merged_data_f$CSCICP03USM665S) print(paste("Correlation between them is", round(correlation, 2)))
...
{r,echo = TRUE} acf(merged_data_f\$RSXFS)
...
{r,echo = TRUE} pacf(merged_data_f\$RSXFS)
...
{r,echo = TRUE} data_length \<- nrow(merged_data_f) \# Use nrow for row count train_length \<- floor(data_length \* 0.8)

# Split the data by rows

train_set \<- merged_data_f\[1:train_length, \] test_set \<- merged_data_f\[(train_length+1):data_length, \]

# Auto ARIMA Model:

{r,echo = TRUE} library(forecast) time_series_data \<- ts(train_set$RSXFS, start=c(1), frequency=12) fit_arima <- auto.arima(time_series_data, xreg = train_set$CSCICP03USM665S) summary(fit_arima)

# Forecast:

{r,echo = TRUE} library(forecast) future_forecast \<- forecast(fit_arima,xreg=test_set$CSCICP03USM665S, h = 12) # Print the forecast print(future_forecast$mean\[1\])

# Vector Autoregression (VAR) Model:

{r,echo = TRUE} library(vars) time_series_data_1 \<- ts(train_set\[, c("RSXFS", "CSCICP03USM665S")\],start = c(1), frequency = 12) var_model \<- VAR(time_series_data_1) summary(var_model)

# Forecast:

{r,echo = TRUE} future_forecast_1 \<- predict(var_model,h = 12) \# Print the forecast print(future_forecast_1$fcst$RSXFS\[1,'fcst'\])

## Cross Validation for Auto ARIMA:

{r,echo = TRUE} \# Auto ARIMA model's accuracy metrics arima_mae \<- mean(abs(future_forecast$mean - test_set$RSXFS)) arima_mse \<- mean((future_forecast$mean - test_set$RSXFS)\^2) arima_rmse \<- sqrt(arima_mse) arima_mape \<- mean(abs((future_forecast$mean - test_set$RSXFS) / test_set\$RSXFS) \* 100) print(paste("Mean Absolute Error:",{arima_mae})) print(paste("Mean Square Error:",{arima_mse})) print(paste("Root Mean Square Error:",{arima_rmse})) print(paste("Mean Absolute Percentage Error:",{arima_mape}))

## Cross Validation for VAR:

{r,echo = TRUE} \# Auto ARIMA model's accuracy metrics var_mae \<- mean(abs(future_forecast_1$fcst$RSXFS\[,'fcst'\] - test_set$RSXFS)) var_mse <- mean((future_forecast_1$fcst$RSXFS[,'fcst'] - test_set$RSXFS)\^2) var_rmse \<- sqrt(var_mse) var_mape \<- mean(abs((future_forecast_1$fcst$RSXFS\[,'fcst'\] - test_set$RSXFS) / test_set$RSXFS) \* 100) print(paste("Mean Absolute Error:",{var_mae})) print(paste("Mean Square Error:",{var_mse})) print(paste("Root Mean Square Error:",{var_rmse})) print(paste("Mean Absolute Percentage Error:",{var_mape}))
